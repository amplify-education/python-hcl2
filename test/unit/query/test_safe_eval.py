...
class TestSafeEval(TestCase):
    ...
    def test_rejects_dunder_chain(self):
        with self.assertRaises(UnsafeExpressionError):
            validate_expression("x.__class__.__subclasses__()")

    def test_rejects_getattr(self):
        with self.assertRaises(UnsafeExpressionError):
            validate_expression("getattr(x, 'y')")

    def test_rejects_hasattr(self):
        with self.assertRaises(UnsafeExpressionError):
            validate_expression("hasattr(x, 'y')")
